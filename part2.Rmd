...
levels(GroupfilterSex$NOC) <- c("Bohemia", "France","Great Britain", "Italy","Lerina","United States")
ggplot(GroupfilterSex, aes(x=NOC, y=Athlete))+ geom_bar(stat = "identity")+ylim(c(0,15))+ylab("Number of Athletes")

GroupfilterSex <- filterSex %>% group_by(NOC,Team) %>% summarise(Athlete = length(unique(ID)))

#Change color for France adn USA to Red(Highlight).
#Change x-axis from NOC to country name.
```

How many country join athlete?
```{r}
